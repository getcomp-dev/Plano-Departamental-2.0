...
const history = function(params){
    models.History.create({
        tabelaModificada: 'GradeCursoExterno',
...
router.post('/', function (req, res, next) {
    ...
    models.GradeCursoExterno.create({
        periodoInicio: req.body.periodoInicio,
        nome: req.body.nome,
        Curso: req.body.Curso
    }).then(function (grade) {
        ioBroadcast(SM.GRADE_CURSO_EXTERNO_CREATED, {'msg': 'Grade criada!', 'Grade': grade})
        console.log('\nRequest de '+req.usuario.nome+'\n')
...
router.get('/', function (req, res, next) {
    models.GradeCursoExterno.findAll().then(function (grades) {
        res.send({
            success: true,
...
router.post('/:id([0-9]+)', function (req, res, next) {
    console.log('\nRequest de '+req.usuario.nome+'\n')
    models.GradeCursoExterno.findOne({
        where: {
            id: req.params.id
        }
    }).then(function (grade) {
        ...
        ioBroadcast(SM.GRADE_CURSO_EXTERNO_UPDATED, {'msg': 'Grade atualizada!', 'Grade': grade})
        console.log('\nRequest de '+req.usuario.nome+'\n')
...
router.delete('/:id([0-9]+)', function (req, res, next) {
    ...
    models.GradeCursoExterno.findOne({
        ...
        ioBroadcast(SM.GRADE_CURSO_EXTERNO_DELETED, {'msg': 'Grade excluída!', 'Grade': grade})
        console.log('\nRequest de '+req.usuario.nome+'\n')
