...
import { USUARIO_FETCHED, SOCKET_USUARIO_CREATED, SOCKET_USUARIO_DELETED, SOCKET_USUARIO_UPDATED } from '../mutation-types'
...
const mutations = {
    ...
    [SOCKET_USUARIO_UPDATED] (state, data) {
        let index = _.findIndex(state.Usuarios, usuario => usuario.id === data.Usuario.id);
...
const actions = {
    fetchAll ({commit}) {
        return new Promise((resolve, reject) => {
            usuarioService.fetchAll().then(response => {
